...
def registrarme(request):
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.save()
            premium = 'y aprovechar tus descuento especiales como cliente premium'
            messages.success(request,f'¡Bienvenido {str(usuario.username)}! {premium}')
...
def misdatos(request):
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            messages.error(request, 'Error al actualizar tu cuenta.')
    ...
    #     messages.error(request, 'Error al actualizar tu cuenta.')
    #     show_form_errors(request, [form_usuario, form_perfil])
    #     print(form_usuario.errors)  # Agregar esta línea
    #     print(form_perfil.errors)   # Agregar esta línea
...
def productos(request, accion, id):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'No fue posible agregar el producto')
    ...
    return render(request, 'core/productos.html', json)
...
def bodega(request):
    ...
    if request.method == 'POST':
        ...
        if cantidad == 1:
            messages.success(request, f'¡Se ha agregado 1 unidad de {producto.nombre} a la bodega')
        else:
            messages.success(request, f'¡Se han agregado {cantidad} unidades de {producto.nombre} a la bodega')
...
def usuarios(request, accion, id):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            usuario.save()  # Guardar el usuario antes de asociar el perfil
            perfil = form_perfil.save(commit=False)
            perfil.usuario = usuario
            perfil.save()
            messages.success(request, f'El usuario "{usuario.first_name}" se guardó correctamente.')
            return redirect('usuarios', accion='actualizar', id=usuario.id)
        else:
            messages.error(request, 'No se pudo guardar el usuario.')
            # Mostrar errores de formulario si es necesario (implementación no proporcionada)
    ...
    context = {
        'form_usuario': form_usuario,
        'form_perfil': form_perfil,
        'usuarios': User.objects.all(),  # Cambiar 'usuario' a 'User.objects.all()' para obtener todos los usuarios
    }
